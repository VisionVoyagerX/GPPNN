...
class PANBlock(nn.Module):
    def __init__(self,
        ...
        self.get_PAN = BasicUnit(
            ms_channels, n_feat, pan_channels, kernel_size)
        self.get_HR_residual = BasicUnit(
            pan_channels, n_feat, ms_channels, kernel_size)
...
class GPPNN(nn.Module):
    def __init__(self,
        ...
        self.lr_blocks = nn.ModuleList(
            [LRBlock(ms_channels, n_feat) for i in range(n_layer)])
        self.pan_blocks = nn.ModuleList(
            [PANBlock(ms_channels, pan_channels, n_feat, 1) for i in range(n_layer)])
    ...
    def forward(self, pan, ms):
        ...
        HR = HR * self.mslr_std + self.mslr_mean

        return HR


if __name__ == "__main__":
    gppnn = GPPNN(4, 1, 64, 8)
    lr = torch.randn(1, 4, 64, 64)
    pan = torch.rand(1, 1, 256, 256)

    print(gppnn(pan, lr).shape)
